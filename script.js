...
(function (self, time) {
  ...
  function pulse() {
    ...
    if (
      ...
      var word = "Stop overthinking. ❤";

    if (
      +new Date().getTime() - lastWord > 21000 &&
      +new Date().getTime() - lastWord < 28000
    )
      var word = "Focus on the present. ❤";

    if (
      +new Date().getTime() - lastWord > 28000 &&
      +new Date().getTime() - lastWord < 35000
    )
      var word = "Don’t think about your problems for now. ❤";

    if (
      +new Date().getTime() - lastWord > 35000 &&
      +new Date().getTime() - lastWord < 42000
    )
      var word = "Everything will be okay ❤";

    if (
      +new Date().getTime() - lastWord > 42000 &&
      +new Date().getTime() - lastWord < 49000
    )
      var word = "I'm Meowish saying. ❤";

    if (+new Date().getTime() - lastWord > 49000)
      var word = "Goodbye and be happy!. ❤";
